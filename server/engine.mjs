// QMM story engine — the shared core, extracted verbatim from server.mjs so BOTH entrypoints
// (the player server and the authoring studio) run the exact same turn semantics and, above all,
// the exact same prompt strings. The prompts ARE the product: tools/engine-parity.mjs byte-checks
// the builders against golden fixtures captured from the pre-extraction server.
//
// Callers inject their surroundings via `deps`:
//   deps = {
//     getModule(id),     // registry lookup ('' / undefined -> the default module)
//     defaultModule,     // id used when a request names no module
//     sessions,          // { loadSession, saveSession, newSession, getOrCreate, clearSession }
//     qlog(entry),       // flight logger (studio logs to its own file, never the player's)
//   }
// The player server passes its live registry + server/sessions.mjs; the studio passes a draft
// registry + a scratch-session store. Engine code never touches the filesystem outside
// loadModules(), and never reads PORT.
...
export const OLLAMA = process.env.OLLAMA || 'http://127.0.0.1:11434';
export const MODEL = process.env.MODEL || 'gemma4:e4b';
export const KEEP_ALIVE = '60m';
export const NUM_CTX = parseInt(process.env.NUM_CTX || '32768', 10);

export const MAX_CHAT_PER_BEAT = 4; // hard cap: the next message after this many chats must advance

// ------------------------------------------------------------ module registry ----
// Each module derives the runtime bits the engine needs from its pack.
export function buildModule(manifest, pack) {
  return {
...
/**
 * Load every module under `dir`. Returns { modules: {id: mod}, errors: [{dir, message}] } —
 * the caller decides what an empty registry means (the player server refuses to boot on it,
 * and refuses to hot-SWAP to it; the studio reports it).
 */
export function loadModules(dir) {
  const modules = {};
  const errors = [];
  for (const d of readdirSync(dir)) {
    try {
      const mdir = join(dir, d);
      ...
      errors.push({ dir: d, message: String(e.message || e) });
      console.error(`[module] FAILED ${d}: ${e.message}`);
    }
  }
...
// ---------------------------------------------------------------- ollama ----
export async function ollamaChat(messages, { format, temperature = 0.8, num_predict = 16384 } = {}) {
  const t0 = Date.now();
...
export function buildRoutePrompt(mod, family, userMessage, tail, exchanges, latencyS) {
...
chat = the message is a question, reaction, emotion, joke, comfort, or thinking out loud — early in a beat, let the conversation breathe.`;
  ...
  return { sys, usr, format };
}

export async function routeMessage(mod, userMessage, family, tail, exchanges, latencyS, qlog = () => {}) {
  const { sys, usr, format } = buildRoutePrompt(mod, family, userMessage, tail, exchanges, latencyS);
...
// -------------------------------------------------------------- generation --
export function stateForPrompt(family, state) {
...
export function buildGeneratePrompt(mod, family, tpl, state, userMessage, tail, loreBlock = '') {
...
Write Yuki's reply now as JSON.`;
  ...
  return { sys, usr, format, minB, maxB };
}

export async function generateBubbles(mod, family, tpl, state, userMessage, tail, loreBlock = '', qlog = () => {}) {
  const { sys, usr, format, minB, maxB } = buildGeneratePrompt(mod, family, tpl, state, userMessage, tail, loreBlock);
...
export function buildChatPrompt(mod, family, state, userMessage, tail, nudgeS, loreBlock = '') {
...
${nudgeS
  ...
  return { sys, usr, format, max: nudgeS ? 1 : 2 };
}

export async function chatAsYuki(mod, family, state, userMessage, tail, nudgeS, loreBlock = '', qlog = () => {}) {
  const { sys, usr, format, max } = buildChatPrompt(mod, family, state, userMessage, tail, nudgeS, loreBlock);
  for (let attempt = 0; attempt < 2; attempt++) {
    const { content, thinking, ms } = await ollamaChat(
      [{ role: 'system', content: sys }, { role: 'user', content: usr }],
...
export function applyUpdates(tpl, prev) {
  const state = { ...prev };
...
export function freshState(mod) {
  return {
...
// ------------------------------------------------------------------- turns --
export function sanitizeTail(tail) {
...
// Resolve (user_id, module_id) from a request body. module defaults to the caller's default.
function resolveIds(deps, body) {
  const userId = String(body.user_id || '').slice(0, 80);
  const moduleId = String(body.module_id || '').slice(0, 80) || deps.defaultModule;
  return { userId, moduleId };
}

export async function runTurn(deps, body, ctx) {
  const { userId, moduleId } = resolveIds(deps, body);
  ...
  const mod = deps.getModule(moduleId);
  ...
  if (/^stop[\s.!?…]*$/i.test(userMessage)) {
    deps.sessions.clearSession(userId, moduleId);
    deps.qlog({ kind: 'stop_word', ...ctx, user_id: userId, module_id: moduleId });
    console.log(`[STOP] user=${userId} module=${moduleId}`);
    return { mode: 'stopped' };
  }

  const st0 = freshState(mod);
  const sess = deps.sessions.getOrCreate(userId, moduleId, () => st0, mod.meta.cold_open.map(t => resolveMacros(t, st0.macro_seed)));
  const state = sess.state;
  const family = mod.familyByFrom[state.current_state];
  if (!family) {
    deps.qlog({ kind: 'turn_rejected', ...ctx, user_id: userId, module_id: moduleId, reason: 'story_over', current_state: state.current_state });
  ...
  const route = await routeMessage(mod, userMessage, family, tail, exchanges, latencyS, deps.qlog);
  ...
  if (route.action !== 'advance' && !forced) {
    const chat = await chatAsYuki(mod, family, state, userMessage, tail, null, lore.block, deps.qlog);
    ...
    sess.state = newState;
    deps.sessions.saveSession(sess);
    ...
    deps.qlog({ kind: 'turn', mode: 'chat', ...base, bubbles: chat.bubbles, gen_ms: chat.ms, gen_fallback: !!chat.fallback, gen_thinking: chat.thinking || '', state_after: newState });
    return {
  ...
  const gen = await generateBubbles(mod, family, tpl, state, userMessage, tail, lore.block, deps.qlog);
  ...
  deps.qlog({ kind: 'turn', mode: 'advance', ...base, template_id: tpl.id, bubbles: gen.bubbles, gen_ms: gen.ms, gen_fallback: !!gen.fallback, gen_thinking: gen.thinking || '', state_after: newState, ending: out.ending || null });
  return out;
}

export async function runNudge(deps, body, ctx) {
  const { userId, moduleId } = resolveIds(deps, body);
  if (!userId) return { error: 'no_user' };
  const mod = deps.getModule(moduleId);
  if (!mod) return { error: 'unknown_module' };
  const sess = deps.sessions.loadSession(userId, moduleId);
  ...
  const chat = await chatAsYuki(mod, family, sess.state, '', tail, quietS, lore.block, deps.qlog);
  chat.bubbles = applyRails(mod.pack, chat.bubbles).bubbles;
  for (const b of chat.bubbles) sess.transcript.push({ who: 'yuki', text: b });
  deps.sessions.saveSession(sess);
  console.log(`[nudge] ${moduleId} ${family.from} quiet=${quietS}s gen=${chat.ms}ms${chat.fallback ? ' FALLBACK' : ''}`);
  deps.qlog({ kind: 'nudge', ...ctx, user_id: userId, module_id: moduleId, from_state: family.from, quiet_s: quietS, bubbles: chat.bubbles, gen_ms: chat.ms, gen_fallback: !!chat.fallback, gen_thinking: chat.thinking || '' });
